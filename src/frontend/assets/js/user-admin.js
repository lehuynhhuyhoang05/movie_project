...
async function updateUser(userData, isUpdate = true) {
  ...
  const url = `http://localhost/movie_project/src/backend/server.php?controller=user&method=${isUpdate ? 'update' : 'create'}&token=Bearer%20${token}`;
...
async function deleteUserAPI(id) {
  ...
  try {
    const url = `http://localhost/movie_project/src/backend/server.php?controller=user&method=delete&id=${id}&token=Bearer%20${token}`;
