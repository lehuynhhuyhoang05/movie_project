document.addEventListener("DOMContentLoaded", function () {
  ...
  function getGenreEnglishName(viName) {
    const found = Object.entries(genreTranslation).find(([en, vi]) => vi.toLowerCase() === viName.toLowerCase());
    return found ? found[0] : viName;
  }
  ...
  movieList.addEventListener("click", async function (e) {
    ...
    if (btn.classList.contains("editMovieBtn")) {
      ...
      const genreNameEn = getGenreEnglishName(genreNameVi);
      titleInput.value = title;
      genreInput.value = genreNameEn;
