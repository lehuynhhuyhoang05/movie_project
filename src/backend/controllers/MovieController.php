...
class MovieController {
    ...
    public function getFavorites() {
        ...
        $user_id = $_SESSION['user_id'];
        $data = $this->movieModel->getFavorites($user_id); // Trả về mảng trực tiếp
        header('Content-Type: application/json');
        echo json_encode(["status" => "success", "data" => $data, "total" => count($data)]);
    }

    public function addWatchHistory($movie_id) {
        session_start();
        if (!isset($_SESSION['user_id'])) {
            http_response_code(401);
            echo json_encode(["status" => "error", "message" => "Unauthorized: Please login"]);
    ...
    public function getWatchHistory() {
        ...
        $user_id = $_SESSION['user_id'];
        $data = $this->movieModel->getWatchHistory($user_id); // Trả về mảng trực tiếp
        header('Content-Type: application/json');
        echo json_encode(["status" => "success", "data" => $data, "total" => count($data)]);
    }
}
